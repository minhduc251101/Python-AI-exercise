...
def visualize_scans_2d(scan1, scan2, T_estimated, T_ground_truth, save=False):
    ...
    ax2.scatter(pts2_est[:, 0], pts2_est[:, 1], s=1, c='tab:orange', label='Transformed current scan (ICP)')
    ax2.set_xlabel('X (m)')
    ax2.set_ylabel('Y (m)')
    ...
    ax3.scatter(pts2_gt[:, 0], pts2_gt[:, 1], s=1, c='tab:green', label='Ground truth aligned scan')
    ax3.set_xlabel('X (m)')
    ax3.set_ylabel('Y (m)')
...
def main():
    ...
    parser.add_argument("--scan_radius", type=float, default=30.0,
    ...
    # --- Overlap check ---
    print(f"\n[5] Checking overlap (using ground truth alignment)...")
    ...
    for key, (name, func) in to_run.items():
        ...
        results[key] = {
            "name": name, "transform": T_est,
            "fitness": result.fitness, "rmse": result.inlier_rmse,
            "trans_error": t_err, "rot_error": r_err,
        }
    ...
    if not args.no_vis and best_key:
        print(f"\n[7] Visualization...")
        # 2D matplotlib plots
        visualize_scans_2d(scan1_proc, scan2_proc,
                           results[best_key]["transform"], T_relative_gt,
                           save=args.save_plots)
        # 3D Open3D viewer
        visualize_scans_3d(scan1_proc, scan2_proc,
                           results[best_key]["transform"], T_relative_gt)
