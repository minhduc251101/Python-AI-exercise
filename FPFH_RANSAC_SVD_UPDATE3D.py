...
def run_fpfh_ransac_svd(source, target,
    ...
    """
    Full pipeline:
        Downsample -> Normals -> FPFH -> Open3D RANSAC SVD
        source -> target
    """
    ...
    # Native Open3D RANSAC C++ implementation (Very FAST)
    result = o3d.pipelines.registration.registration_ransac_based_on_feature_matching(
        ...
        criteria=o3d.pipelines.registration.RANSACConvergenceCriteria(max_iterations, 500)
    )

    if result.fitness == 0.0:
        return None

    # Reformat the result to match the expected dictionary output format for the rest of your script
    ...
    return {
        ...
        "t": T_est[:3, 3:4],
        "corr_pairs": corr_set,
        "inlier_mask": np.ones(len(corr_set), dtype=bool), # dummy mask since all returned are inliers
        "num_inliers": len(corr_set),
        "inlier_ratio": result.fitness, # Note: Open3D fitness is relatively to total source down points
...
#                         sample_size=4,
#                         edge_ratio=0.9,
#                         random_seed=0): # MUST INCLUDE random_seed TO PREVENT TYPE ERRORS FROM MAIN
#     """
#     Full pipeline:
#         Downsample -> Normals -> FPFH -> Open3D RANSAC -> 3D Arun
#         source -> target
#     """
#     if distance_threshold is None:
#         distance_threshold = voxel_size * 3.0
...
#     # Tìm ra toàn bộ lượng match tiềm năng (để báo cáo form cũ)
#     corr_pairs = build_mutual_fpfh_correspondences(source_fpfh, target_fpfh)
#     if len(corr_pairs) < sample_size:
#         return None

#     # Native Open3D RANSAC C++ implementation (Rất nhanh và Không bị lật trục)
#     result = o3d.pipelines.registration.registration_ransac_based_on_feature_matching(
#         source_down, target_down, source_fpfh, target_fpfh,
#         mutual_filter=True,
#         max_correspondence_distance=distance_threshold,
...
#     if result.fitness == 0.0 or len(result.correspondence_set) < 3:
#         return None

#     # Lấy những inliers ưu tú nhất mà C++ Open3D lọc ra được
#     corr_set = np.asarray(result.correspondence_set)
#     src_points = np.asarray(source_down.points)
#     tgt_points = np.asarray(target_down.points)
    
#     src_inliers = src_points[corr_set[:, 0]]
#     tgt_inliers = tgt_points[corr_set[:, 1]]

#     # Chạy hàm arun 3D của bạn trực tiếp lên các inliers này để tính Toán T_final thuần túy
#     T_final, R_final, t_final = arun(src_inliers.T, tgt_inliers.T)

#     # Đánh giá lại inlier_mask trên TOÀN BỘ list corr_pairs để log xuất ra tương thích 100% với form cũ
#     src_corr_pts = src_points[corr_pairs[:, 0]]
#     tgt_corr_pts = tgt_points[corr_pairs[:, 1]]
...
#         "transformation": T_final,
#         "R": R_final,
#         "t": t_final.reshape(3, 1), # Ép về shape chuẩn
#         "corr_pairs": corr_pairs,
#         "inlier_mask": final_inlier_mask,
#         "num_inliers": int(np.sum(final_inlier_mask)),
#         "inlier_ratio": float(np.sum(final_inlier_mask)) / float(len(corr_pairs)) if len(corr_pairs) > 0 else 0.0,
#         "rmse": float(np.sqrt(np.mean(final_residuals[final_inlier_mask] ** 2))) if np.any(final_inlier_mask) else np.inf,
#         "source_down": source_down,
#         "target_down": target_down,
#         "source_fpfh": source_fpfh,
