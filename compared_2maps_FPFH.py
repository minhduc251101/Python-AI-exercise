...
def draw_result(source, target, T, title):
    ...
    o3d.visualization.draw_geometries([tgt, src], window_name=title)
...
for i in range(
    0, fpfh_array_source.shape[1], 2000
):  # using tuple 33 (0)    ,    65553(1)
...
for i in range(0, corres_np.shape[0], 1000):
    src_idx = corres_np[
        i, 0
    ]  # first column, idx start from 0 and first column is from 0 and second is from 1
    tgt_idx = corres_np[i, 1]  # second column
    print(
        f"FPFH of matched correspondence pair {i}:source point[{src_idx}]<-> target point[{tgt_idx}]"
    )
...
fig.suptitle(
    f"FPFH Descriptor for Point {point_idx} (33 bins = 11 + 11 + 11)",
    fontsize=13,
    fontweight="bold",
)
plt.tight_layout()
